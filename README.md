...
Earthworm gồm 5 nhóm thành phần chính:
...
```env
DATABASE_URL="postgres://postgres:<POSTGRES_PASSWORD>@db:5432/earthworm"
...
- `earthworm-client`
- `earthworm-dash`
- `earthworm-ops` (dùng cho migrate/seed)
...
- `docker.io/<namespace>/earthworm-api:latest`
- `docker.io/<namespace>/earthworm-client:latest`
- `docker.io/<namespace>/earthworm-dash:latest`
- `docker.io/<namespace>/earthworm-ops:latest`
...
```bash
mkdir -p /opt/earthworm/apps/api
cd /opt/earthworm
```
...
### 11.4 `Cannot find module '@earthworm/schema/dist/index.js'`

Nguyên nhân: schema chưa build trước seed trong image cũ.  
Khắc phục: rebuild/pull image `earthworm-ops` mới rồi chạy lại seed.
